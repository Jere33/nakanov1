...
client.on('qr', qr => {
   ...
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('qr already scan.subscribe','white'),color('YOU','red'),color('TUBE','white'),color('jereganz','yellow'))
})
           ...
           return false
			...
			switch(command) { 
				...
				case 'neontext'
                	if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					anu = await fecthJson(`http://lolhuman.herokuapp.com/api/textprome/neon?apikey=WEMPYGANSS&text=LoLHuman${aruga}`)
					reply('Wait Kak...')
					buffer = await getBuffer(anu.result.url)
					naka.sendMessage(froom, buffer,image, {quoted.mek})
					await limitAdd(sender)
					break
case 'sumery'
                	if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					anu = await fecthJson(`https://arugaz.my.id/api/textpro/sandsummery?text=${aruga}`)
					reply('Wait Kak...')
					buffer = await getBuffer(anu.result.url)
					naka.sendMessage(froom, buffer,image, {quoted.mek})
					await limitAdd(sender)
